const useTikker = () => {
    return `
    <div class="w-full overflow-hidden flex bg-black">
        <div class="newsletter-container">
            <div class="newsletter-item">
                <span class="text-xs">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                        viewBox="0 0 54 60" fill="none">
                        <path
                            d="M27.4024 59.1687C30.778 39.262 36.5949 32.876 54.0039 29.8778C54.0039 29.8778 38.9916 26.8949 34.0039 19.3643C29.0161 11.8337 29.6029 7.23716 27.3046 0C25.2508 6.65037 25.5658 14.028 19.9207 20.2934C14.324 26.5051 7.15789 28.0685 0.703125 29.7311C18.3566 33.202 23.9832 39.9368 27.4024 59.1687Z"
                            fill="#FFBB00" />
                    </svg>&nbsp; 
                    Crafted by the Top 1% of Industry Experts
                </span>
            </div>
            <div class="newsletter-item">
                <span class="text-xs">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                        viewBox="0 0 54 60" fill="none">
                        <path
                            d="M27.4024 59.1687C30.778 39.262 36.5949 32.876 54.0039 29.8778C54.0039 29.8778 38.9916 26.8949 34.0039 19.3643C29.0161 11.8337 29.6029 7.23716 27.3046 0C25.2508 6.65037 25.5658 14.028 19.9207 20.2934C14.324 26.5051 7.15789 28.0685 0.703125 29.7311C18.3566 33.202 23.9832 39.9368 27.4024 59.1687Z"
                            fill="#FFBB00" />
                    </svg>&nbsp; 
                    6-Month Internship Opportunity
                </span>
            </div>
            <div class="newsletter-item">
                <span class="text-xs">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                        viewBox="0 0 54 60" fill="none">
                        <path
                            d="M27.4024 59.1687C30.778 39.262 36.5949 32.876 54.0039 29.8778C54.0039 29.8778 38.9916 26.8949 34.0039 19.3643C29.0161 11.8337 29.6029 7.23716 27.3046 0C25.2508 6.65037 25.5658 14.028 19.9207 20.2934C14.324 26.5051 7.15789 28.0685 0.703125 29.7311C18.3566 33.202 23.9832 39.9368 27.4024 59.1687Z"
                            fill="#FFBB00" />
                    </svg>&nbsp; 
                    Comprehensive Job Placement Support
                </span>
            </div>
            <div class="newsletter-item">
                <span class="text-xs">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                        viewBox="0 0 54 60" fill="none">
                        <path
                            d="M27.4024 59.1687C30.778 39.262 36.5949 32.876 54.0039 29.8778C54.0039 29.8778 38.9916 26.8949 34.0039 19.3643C29.0161 11.8337 29.6029 7.23716 27.3046 0C25.2508 6.65037 25.5658 14.028 19.9207 20.2934C14.324 26.5051 7.15789 28.0685 0.703125 29.7311C18.3566 33.202 23.9832 39.9368 27.4024 59.1687Z"
                            fill="#FFBB00" />
                    </svg>&nbsp; 
                    Learn from Industry Practitioners
                </span>
            </div>
            <div class="newsletter-item">
                <span class="text-xs">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                        viewBox="0 0 54 60" fill="none">
                        <path
                            d="M27.4024 59.1687C30.778 39.262 36.5949 32.876 54.0039 29.8778C54.0039 29.8778 38.9916 26.8949 34.0039 19.3643C29.0161 11.8337 29.6029 7.23716 27.3046 0C25.2508 6.65037 25.5658 14.028 19.9207 20.2934C14.324 26.5051 7.15789 28.0685 0.703125 29.7311C18.3566 33.202 23.9832 39.9368 27.4024 59.1687Z"
                            fill="#FFBB00" />
                    </svg>&nbsp; 
                    Dive into Applied AI Every Semester
                </span>
            </div>
            <div class="newsletter-item">
                <span class="text-xs">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                        viewBox="0 0 54 60" fill="none">
                        <path
                            d="M27.4024 59.1687C30.778 39.262 36.5949 32.876 54.0039 29.8778C54.0039 29.8778 38.9916 26.8949 34.0039 19.3643C29.0161 11.8337 29.6029 7.23716 27.3046 0C25.2508 6.65037 25.5658 14.028 19.9207 20.2934C14.324 26.5051 7.15789 28.0685 0.703125 29.7311C18.3566 33.202 23.9832 39.9368 27.4024 59.1687Z"
                            fill="#FFBB00" />
                    </svg>&nbsp; 
                    Engage in Real-world UX Design Projects
                </span>
            </div>
            <div class="newsletter-item">
                <span class="text-xs">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                        viewBox="0 0 54 60" fill="none">
                        <path
                            d="M27.4024 59.1687C30.778 39.262 36.5949 32.876 54.0039 29.8778C54.0039 29.8778 38.9916 26.8949 34.0039 19.3643C29.0161 11.8337 29.6029 7.23716 27.3046 0C25.2508 6.65037 25.5658 14.028 19.9207 20.2934C14.324 26.5051 7.15789 28.0685 0.703125 29.7311C18.3566 33.202 23.9832 39.9368 27.4024 59.1687Z"
                            fill="#FFBB00" />
                    </svg>&nbsp; 
                    Crafted by the Top 1% of Industry Experts
                </span>
            </div>
            <div class="newsletter-item">
                <span class="text-xs">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                        viewBox="0 0 54 60" fill="none">
                        <path
                            d="M27.4024 59.1687C30.778 39.262 36.5949 32.876 54.0039 29.8778C54.0039 29.8778 38.9916 26.8949 34.0039 19.3643C29.0161 11.8337 29.6029 7.23716 27.3046 0C25.2508 6.65037 25.5658 14.028 19.9207 20.2934C14.324 26.5051 7.15789 28.0685 0.703125 29.7311C18.3566 33.202 23.9832 39.9368 27.4024 59.1687Z"
                            fill="#FFBB00" />
                    </svg>&nbsp; 
                    6-Month Internship Opportunity
                </span>
            </div>
            <div class="newsletter-item">
                <span class="text-xs">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                        viewBox="0 0 54 60" fill="none">
                        <path
                            d="M27.4024 59.1687C30.778 39.262 36.5949 32.876 54.0039 29.8778C54.0039 29.8778 38.9916 26.8949 34.0039 19.3643C29.0161 11.8337 29.6029 7.23716 27.3046 0C25.2508 6.65037 25.5658 14.028 19.9207 20.2934C14.324 26.5051 7.15789 28.0685 0.703125 29.7311C18.3566 33.202 23.9832 39.9368 27.4024 59.1687Z"
                            fill="#FFBB00" />
                    </svg>&nbsp; 
                    Comprehensive Job Placement Support
                </span>
            </div>
            <div class="newsletter-item">
                <span class="text-xs">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                        viewBox="0 0 54 60" fill="none">
                        <path
                            d="M27.4024 59.1687C30.778 39.262 36.5949 32.876 54.0039 29.8778C54.0039 29.8778 38.9916 26.8949 34.0039 19.3643C29.0161 11.8337 29.6029 7.23716 27.3046 0C25.2508 6.65037 25.5658 14.028 19.9207 20.2934C14.324 26.5051 7.15789 28.0685 0.703125 29.7311C18.3566 33.202 23.9832 39.9368 27.4024 59.1687Z"
                            fill="#FFBB00" />
                    </svg>&nbsp; 
                    Learn from Industry Practitioners
                </span>
            </div>
            <div class="newsletter-item">
                <span class="text-xs">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                        viewBox="0 0 54 60" fill="none">
                        <path
                            d="M27.4024 59.1687C30.778 39.262 36.5949 32.876 54.0039 29.8778C54.0039 29.8778 38.9916 26.8949 34.0039 19.3643C29.0161 11.8337 29.6029 7.23716 27.3046 0C25.2508 6.65037 25.5658 14.028 19.9207 20.2934C14.324 26.5051 7.15789 28.0685 0.703125 29.7311C18.3566 33.202 23.9832 39.9368 27.4024 59.1687Z"
                            fill="#FFBB00" />
                    </svg>&nbsp; 
                    Dive into Applied AI Every Semester
                </span>
            </div>
            <div class="newsletter-item">
                <span class="text-xs">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                        viewBox="0 0 54 60" fill="none">
                        <path
                            d="M27.4024 59.1687C30.778 39.262 36.5949 32.876 54.0039 29.8778C54.0039 29.8778 38.9916 26.8949 34.0039 19.3643C29.0161 11.8337 29.6029 7.23716 27.3046 0C25.2508 6.65037 25.5658 14.028 19.9207 20.2934C14.324 26.5051 7.15789 28.0685 0.703125 29.7311C18.3566 33.202 23.9832 39.9368 27.4024 59.1687Z"
                            fill="#FFBB00" />
                    </svg>&nbsp; 
                    Engage in Real-world UX Design Projects
                </span>
            </div>
            <div class="newsletter-item">
                <span class="text-xs">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                        viewBox="0 0 54 60" fill="none">
                        <path
                            d="M27.4024 59.1687C30.778 39.262 36.5949 32.876 54.0039 29.8778C54.0039 29.8778 38.9916 26.8949 34.0039 19.3643C29.0161 11.8337 29.6029 7.23716 27.3046 0C25.2508 6.65037 25.5658 14.028 19.9207 20.2934C14.324 26.5051 7.15789 28.0685 0.703125 29.7311C18.3566 33.202 23.9832 39.9368 27.4024 59.1687Z"
                            fill="#FFBB00" />
                    </svg>&nbsp; 
                    Crafted by the Top 1% of Industry Experts
                </span>
            </div>
            <div class="newsletter-item">
                <span class="text-xs">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                        viewBox="0 0 54 60" fill="none">
                        <path
                            d="M27.4024 59.1687C30.778 39.262 36.5949 32.876 54.0039 29.8778C54.0039 29.8778 38.9916 26.8949 34.0039 19.3643C29.0161 11.8337 29.6029 7.23716 27.3046 0C25.2508 6.65037 25.5658 14.028 19.9207 20.2934C14.324 26.5051 7.15789 28.0685 0.703125 29.7311C18.3566 33.202 23.9832 39.9368 27.4024 59.1687Z"
                            fill="#FFBB00" />
                    </svg>&nbsp; 
                    6-Month Internship Opportunity
                </span>
            </div>
            <div class="newsletter-item">
                <span class="text-xs">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                        viewBox="0 0 54 60" fill="none">
                        <path
                            d="M27.4024 59.1687C30.778 39.262 36.5949 32.876 54.0039 29.8778C54.0039 29.8778 38.9916 26.8949 34.0039 19.3643C29.0161 11.8337 29.6029 7.23716 27.3046 0C25.2508 6.65037 25.5658 14.028 19.9207 20.2934C14.324 26.5051 7.15789 28.0685 0.703125 29.7311C18.3566 33.202 23.9832 39.9368 27.4024 59.1687Z"
                            fill="#FFBB00" />
                    </svg>&nbsp; 
                    Comprehensive Job Placement Support
                </span>
            </div>
            <div class="newsletter-item">
                <span class="text-xs">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                        viewBox="0 0 54 60" fill="none">
                        <path
                            d="M27.4024 59.1687C30.778 39.262 36.5949 32.876 54.0039 29.8778C54.0039 29.8778 38.9916 26.8949 34.0039 19.3643C29.0161 11.8337 29.6029 7.23716 27.3046 0C25.2508 6.65037 25.5658 14.028 19.9207 20.2934C14.324 26.5051 7.15789 28.0685 0.703125 29.7311C18.3566 33.202 23.9832 39.9368 27.4024 59.1687Z"
                            fill="#FFBB00" />
                    </svg>&nbsp; 
                    Learn from Industry Practitioners
                </span>
            </div>
            <div class="newsletter-item">
                <span class="text-xs">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                        viewBox="0 0 54 60" fill="none">
                        <path
                            d="M27.4024 59.1687C30.778 39.262 36.5949 32.876 54.0039 29.8778C54.0039 29.8778 38.9916 26.8949 34.0039 19.3643C29.0161 11.8337 29.6029 7.23716 27.3046 0C25.2508 6.65037 25.5658 14.028 19.9207 20.2934C14.324 26.5051 7.15789 28.0685 0.703125 29.7311C18.3566 33.202 23.9832 39.9368 27.4024 59.1687Z"
                            fill="#FFBB00" />
                    </svg>&nbsp; 
                    Dive into Applied AI Every Semester
                </span>
            </div>
            <div class="newsletter-item">
                <span class="text-xs">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                        viewBox="0 0 54 60" fill="none">
                        <path
                            d="M27.4024 59.1687C30.778 39.262 36.5949 32.876 54.0039 29.8778C54.0039 29.8778 38.9916 26.8949 34.0039 19.3643C29.0161 11.8337 29.6029 7.23716 27.3046 0C25.2508 6.65037 25.5658 14.028 19.9207 20.2934C14.324 26.5051 7.15789 28.0685 0.703125 29.7311C18.3566 33.202 23.9832 39.9368 27.4024 59.1687Z"
                            fill="#FFBB00" />
                    </svg>&nbsp; 
                    Engage in Real-world UX Design Projects
                </span>
            </div>
            
        </div>
    </div>
    `;
}

const tikkerContainers = (containerId) => {
    document.getElementById(containerId).innerHTML = useTikker();
}

const array = ['tikker1', 'tikker2', 'tikker3', 'tikker4', 'tikker5']

array.forEach(element => {
    tikkerContainers(element)
});